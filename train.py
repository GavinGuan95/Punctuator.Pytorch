...
parser.add_argument('--model-path', default='models/test_deepspeech_final.pth',
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            if not args.silent:
                st = 'Epoch: [{0}][{1}/{2}]\t Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t Data {data_time.val:.3f} ({data_time.avg:.3f})\t Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(
                    (epoch + 1), (i + 1), len(train_sampler), batch_time=batch_time, data_time=data_time, loss=losses)
                print(st)
                with open("./gavin_train.log", "a+") as fh:
                    fh.write(st + "\n")
        ...
        st = 'Training Summary Epoch: [{0}]\t Time taken (s): {epoch_time:.0f}\t Average Loss {loss:.3f}\t'.format(epoch + 1, epoch_time=epoch_time, loss=avg_loss)
        print(st)
        with open("./gavin_train.log", "a+") as fh:
            fh.write(st + "\n")
        ...
        try:
            if epoch % 5 == 0:
                epoch_model_path = "models/deepspeech_final" + str(epoch) + ".pth"
                print("Epoch: {}, saving to {}".format(epoch, epoch_model_path))
                torch.save(DeepSpeech.serialize(model, optimizer=optimizer, epoch=epoch, loss_results=loss_results,
                                                wer_results=wer_results, cer_results=cer_results)
                           , epoch_model_path)
        except:
